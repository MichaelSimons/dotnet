...
if [[ "$sourceOnly" == "true" ]]; then
  ...
  # Initialize source-only toolset (including custom SDK setup and source-built resolver)
  source_only_toolset_init "$scriptroot" "$customSdkDir" "$CUSTOM_PACKAGES_DIR" "$NUGET_PACKAGES" "false" "${properties[@]}"
fi
